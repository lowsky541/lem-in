...
// the timeout.
const defaultTimeout time.Duration = 1<<63 - 1
...
func run(file *os.File) result {
	farm, err := core.ParseFromReader(file)
	if err != nil {
	...
	testCases := []testCase{
		{
			...
			err:      core.ErrCommandIsNotAllowed,
		},

		{
			filepath: "../samples/badexample00",
			err:      core.ErrInvalidAntCount,
		},
		{
			filepath: "../samples/badexample01",
			err:      core.ErrInsaneFarm,
		},
	}
